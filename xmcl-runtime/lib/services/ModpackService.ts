...
import { ensureDir, remove, stat, unlink, writeFile } from 'fs-extra'
  ...
  async exportModpack(options: ExportModpackOptions) {
    ...
    for (const file of files) {
      ...
      if (file.path.startsWith('mods/')) {
        ...
        if (mod && mod.curseforge && directives[file.path] === 'curseforge') {
        ...
        } else if (mod?.modrinth && directives[file.path] === 'modrinth') {
          // modrinth not allowed to include curseforge source by regulation
          const availableDownloads = mod.uri.filter(u => u.startsWith('http') && u.indexOf('edge.forgecdn.net') === -1 && u.indexOf('media.forgecdn.net') === -1)
        ...
        if (resourcepack && resourcepack.curseforge && directives[file.path] === 'curseforge') {
        ...
        } else if (resourcepack?.modrinth && directives[file.path] === 'modrinth') {
          // modrinth not allowed to include curseforge source by regulation
          const availableDownloads = resourcepack.uri.filter(u => u.startsWith('http') && u.indexOf('edge.forgecdn.net') === -1 && u.indexOf('media.forgecdn.net') === -1)
          modrinthManifest?.files.push({
            path: file.path,
            hashes: {
              sha1: await this.worker().checksum(filePath, 'sha1'),
