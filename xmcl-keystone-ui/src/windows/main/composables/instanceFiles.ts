...
export function useInstanceFileNodesFromLocal(local: Ref<InstanceFile[]>, options: { curseforge: boolean; modrinth: boolean }) {
  function getFileNode(f: InstanceFile): InstanceFileNode<InstanceFileExportData> {
    return reactive({
      ...
      data: {
        ...
        canExport: computed(() => (options.modrinth && (!!f.modrinth || !!f.downloads?.length)) || (options.curseforge && !!f.curseforge)),
      },
      children: undefined,
    })
  }
